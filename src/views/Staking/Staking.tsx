  ...
  return (
    <Switch>
      <Page>
        ...
        <StyledCardHeader>
            <Label text={`Welcome to BSCXSafe, stake BSCX to earn BSCX.`}/>
        </StyledCardHeader>
        <div style={{fontWeight: 'bold', fontSize: 22, color: '#ffffff'}}>
            BSCXSafe Currently has <span style={{color: '#4caf50', fontSize: 30}}>{parseFloat(getBalanceNumber(new BigNumber(totalSupply)).toFixed(2)).toLocaleString('en-US')}</span> BSCX Staked
        </div>
        <Spacer size="md" />
        {!!account ? (
          <>
